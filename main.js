...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  */
  ...
  vec2 map(vec3 p)
  {
	  ...
	  float z = 0.3;
	  float trk = 1.4;
	  float dspAmp = 0.01 + prm1*0.5;
	  for(int i = 0; i < 5; i++)
	  {
		  p += sin(p.zxy*0.75*trk + iTime*trk*.90)*dspAmp;
  ...
  vec4 render( in vec3 ro, in vec3 rd, float time )
  {
	  ...
	  for(int i=0; i<190; i++)
	  {
		  ...
		  vec3 pos = ro + t*rd;
		  vec2 mpv = map(pos);
		  float den = clamp(mpv.x-0.1,0.9,1.)*0.92;
		  float dn = clamp((mpv.x + 2.),0.,9.);
  ...
  {	
	  ...
	  prm1 = smoothstep(-0.4, 0.4,sin(iTime*0.3)*0.);
	  vec4 scn = render(ro, rd, time);
		  
	  vec3 col = scn.rgb;
	  col = iLerp(col.bgr, col.rgb, clamp(1.-prm1,0.05,1.));
	  
	  col = pow(col, vec3(.55,0.95,0.6))*vec3(1.,.97,.9);
